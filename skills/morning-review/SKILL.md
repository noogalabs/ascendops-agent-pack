...
```bash
# Default to continuing yesterday's daily_focus if set, otherwise "Fleet stability + maintenance ops"
...
# Proceed with cascade using DEFAULT_FOCUS
# Notify David: "No reply received — defaulting to: [focus]. Reply anytime to redirect."
```
...
1. Send briefing via direct Telegram API using WebFetch (bypass cortextos bus)
2. Send goal cascade notices via `cortextos bus send-message <agent>` — uses file I/O, not shell
3. Mark morning review as "degraded — shell broken" in Telegram
4. Alert David immediately: "Morning review sent but shell is broken — some steps skipped"
5. Retry full cascade when shell recovers (check every 30min)

## Concurrent Cron De-duplication

Morning review runs ONCE per day. If the cron fires again due to session restart:
```bash
# Check if morning review already ran today
